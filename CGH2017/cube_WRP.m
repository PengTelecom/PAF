...
%Pour obtenir un cube de référence, avec des paramètres intéressants, utiliser : cube_WRP(0.5,770,0.6,24000,0,0,30,0,0,0,1080).
...
[X, Y] = meshgrid(pas_pixel*[-largeur_WRP/2.:largeur_WRP/2.-1], pas_pixel*[-largeur_WRP/2.:largeur_WRP/2.-1]);    %Representation mathematique de la matrice WRP
...
for m=-N/2:1:N/2
	scalkrp1=(sqrt((((x0+(-N/2))*pas_reseau-X).^2 + ((y0+(-N/2))*pas_reseau-Y).^2  + (m*pas_reseau_z-R).^2))*k);    %arete dessous, profondeur, gauche.
	M = a.*exp(-1i*scalkrp1);
...
amplitude_tampon(1:largeur_WRP,1:largeur_WRP) = 0;
for m=-N/2:1:N/2
	scalkrp2=(sqrt((((x0+(N/2))*pas_reseau-X).^2 + ((y0+(N/2))*pas_reseau-Y).^2 + (m*pas_reseau_z-R).^2))*k);   %arete en profondeur haut droite
	M = a.*exp(-1i*scalkrp2);
	amplitude_tampon = amplitude_tampon + M;
end;
